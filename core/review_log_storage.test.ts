...
import { extractReviewLogsFromCSV, update } from "./review_log_storage.ts";
import type { ReviewLogInput } from "ts-fsrs";

Deno.test("update should yield the title block", () => {
  const text = "Sample Title\n";
  const reviewLogs = new Map<`${string}-${number}`, ReviewLogInput>();
...
Deno.test("extractReviewLogsFromCSV should parse CSV correctly", async () => {
  const result = await extractReviewLogsFromCSV(
    ReadableStream.from([
      "id,ord,rating,state,due,stability,difficulty,elapsed_days,last_elapsed_days,scheduled_days,review\n",
      "note1,1,3,2,1627849200000,0.5,0.3,10,5,15,1627849200000\n",
      "note2,2,4,3,1627849200000,0.7,0.5,12,7,17,1627849200000\n",
